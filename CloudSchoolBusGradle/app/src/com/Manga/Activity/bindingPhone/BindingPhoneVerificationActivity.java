...
public class BindingPhoneVerificationActivity extends BaseActivity {
	/**
	 * 超时
	 */
	/**
		...
		public boolean handleMessage(Message mess) {
			...
			case SHOWPROGRESS:
				if (progressDialog == null) {
					progressDialog = new ProgressDialog(BindingPhoneVerificationActivity.this);
					progressDialog.setMessage(getResources().getString(R.string.init_view));
				}
				progressDialog.show();
				break;
			case NETOUT:
				Toast.makeText(BindingPhoneVerificationActivity.this, R.string.binding_message_out, Toast.LENGTH_SHORT)
						.show();
				break;
			case SHOW:
			...
			case MODIFIMOREPHONEFAIL:
				Toast.makeText(BindingPhoneVerificationActivity.this, R.string.binding_message_fail, Toast.LENGTH_SHORT)
						.show();
				progressDialog.dismiss();
				break;
			case MODIFIONLINESUCCES:
				Toast.makeText(BindingPhoneVerificationActivity.this, R.string.binding_message_success,
				...
				break;
			case PHONETYPEFAIL:
				Toast.makeText(BindingPhoneVerificationActivity.this, R.string.binding_message_fail_type,
						Toast.LENGTH_SHORT).show();
				progressDialog.dismiss();
				break;
			case MESSAGEFAIL:
				Toast.makeText(BindingPhoneVerificationActivity.this, R.string.binding_message_fail_message,
						Toast.LENGTH_SHORT).show();
				progressDialog.dismiss();
				break;
			case MODIFIONLINESUCCESS:
				...
				progressDialog.dismiss();
				UpdateDB();
				ActivityUtil.close(BindingPhoneVerificationActivity.this);
			}
			return false;
		}
	});
	...
	public void reSend(View view) {
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				// TODO Auto-generated method stub
				if (HttpUtil.isNetworkConnected(BindingPhoneVerificationActivity.this)) {
					Params param = new Params("Bind", map);
					Result result = HttpUtil.httpGet(BindingPhoneVerificationActivity.this, param);
					if (result == null) {
	...
	public void next(View view) {
		...
		thread2 = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtil.isNetworkConnected(BindingPhoneVerificationActivity.this)) {
					Params param = new Params("Bind", map);
					Result result = HttpUtil.httpPost(BindingPhoneVerificationActivity.this, param);
					if (result == null) {
						handler.sendEmptyMessage(MODIFIMOREPHONEFAIL);
					} else if ("1".equals(result.getCode())) {
	...
	public void UpdateDB() {
		try {
			...
			sql.update("student_info", values, "mobile=?", new String[] { strMobile });

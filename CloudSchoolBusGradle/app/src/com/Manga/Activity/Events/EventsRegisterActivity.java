...
public class EventsRegisterActivity extends BaseActivity {
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.what) {
			case OUTTIME:
				Toast.makeText(EventsRegisterActivity.this, R.string.out_time, Toast.LENGTH_SHORT).show();
				break;
			case HAVENODATA:
				Toast.makeText(EventsRegisterActivity.this, R.string.have_no_new_data, Toast.LENGTH_SHORT).show();
	...
	private void getRegisterInfo(final String startid, final String endid, final String myevents, final String signup,
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				if (HttpUtil.isNetworkConnected(EventsRegisterActivity.this)) {
					HashMap<String, String> map = new HashMap<String, String>();
					map.put("startid", startid);
					map.put("endid", endid);
					map.put("myevents", myevents);
					map.put("signup", signup);
					Result result = HttpUtil.httpGet(EventsRegisterActivity.this, new Params("eventslist", map));
					if (result == null) {
						handler.sendEmptyMessage(OUTTIME);
					} else if ("1".equals(result.getCode())) {
						try {
							JSONArray array = new JSONArray(result.getContent());
							DB db = new DB(EventsRegisterActivity.this);
							SQLiteDatabase sql = db.getReadableDatabase();

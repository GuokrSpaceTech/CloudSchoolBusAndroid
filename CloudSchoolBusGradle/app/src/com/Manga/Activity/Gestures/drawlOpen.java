...
public class drawlOpen extends View{
 ...
 //触摸事件
	...
	public boolean onTouchEvent(MotionEvent event) {
		fStartX = (float) (ActivityUtil.select2.getStartX(1) + ActivityUtil.select2.getWidth()/2.0);
		fStartY = (float) (ActivityUtil.select2.getStartY(1) + ActivityUtil.select2.getHeight()/2.0);
		fDisX = ActivityUtil.select2.getStartX(2)-ActivityUtil.select2.getStartX(1);
		fDisY = ActivityUtil.select2.getStartX(4)-ActivityUtil.select2.getStartX(1);
		fFormX = ActivityUtil.select2.getWidth()/2-5;
		fFormY = ActivityUtil.select2.getHeight()/2-5;
 		if (event.getAction()==MotionEvent.ACTION_MOVE) {//如果拖动
 			paint.setXfermode(new PorterDuffXfermode(Mode.CLEAR));
 			//paint.setXfermode(new PorterDuffXfermode(Mode.CLEAR));  
 			canvas.drawPaint(paint); 

 			//paint.setStrokeWidth(50);//笔宽10像素
 			//canvas.drawLine(100, 100, movend_x, movend_y, paint);//画线
 			paint.setXfermode(new PorterDuffXfermode(Mode.SRC));
 			paint.setStrokeWidth(20);//笔宽5像素
 				...
 				for(int jj=0; jj<3; jj++){
 					...
 					if((Math.abs(event.getX() - (fStartX+ii*fDisX))<fFormX)&&(Math.abs(event.getY() - (fStartY+jj*fDisY))<fFormY)){
 						...
 						if(!blnIsHave){
 							...
 							ActivityUtil.select2.ChangePicBg(ii+jj*3+1+"");
 						}
 					}
 				}
 			}
...
// 			}
 		...
 		if(event.getAction() == MotionEvent.ACTION_UP){
 			...
 			if(nSelectArr.size()<4 && nSelectArr.size()>=0){
 				ActivityUtil.select2.UpdateNumber();
 	 			ActivityUtil.select2.ChangePicBg("0");
 	 			paint.setXfermode(new PorterDuffXfermode(Mode.CLEAR));
 	 			canvas.drawPaint(paint); 
 	 			mov_x =0;
 	 			mov_x =0;
 			...
 			ActivityUtil.select2.UpdateNewLock(strLock);
 			ActivityUtil.select2.ChangePicBg("0");
 			paint.setXfermode(new PorterDuffXfermode(Mode.CLEAR));
 			canvas.drawPaint(paint); 
 			mov_x =0;
	...
	public drawlOpen(Context context, AttributeSet attrs, int defStyle) {  
	    super(context, attrs, defStyle);  
	     // TODO Auto-generated constructor stub  
	}  
	public drawlOpen(Context context, AttributeSet attrs) {  
	    super(context, attrs);  
	}

...
package com.android.support.jhf.network.loopj.android.http;
...
public abstract class AsyncHttpResponseHandler implements
	...
	private IErrorListener mIErrorListener;
	...
	public void setErrorListener(IErrorListener iErrorListener) {
		mIErrorListener = iErrorListener;
	}
	 ...
	 */
	public void onProgress(int bytesWritten, int totalSize) {
		Log.v(LOG_TAG, String.format("Progress %d from %d (%d%%)",
				bytesWritten, totalSize,
				(totalSize > 0) ? (bytesWritten / totalSize) * 100 : -1));
	}
	...
	public abstract void onSuccess(int statusCode, Header[] headers,
			byte[] responseBody, HttpResponse response);
	...
	final public void sendSuccessMessage(int statusCode, Header[] headers,
			byte[] responseBytes, HttpResponse response) {
		sendMessage(obtainMessage(SUCCESS_MESSAGE, new Object[] { statusCode,
				headers, responseBytes ,response}));
	}
	...
	final public void sendStartMessage(AsyncHttpRequest asyncHttpRequest) {
		sendMessage(obtainMessage(START_MESSAGE, asyncHttpRequest));
	}

	final public void sendFinishMessage(AsyncHttpRequest asyncHttpRequest) {
		sendMessage(obtainMessage(FINISH_MESSAGE, asyncHttpRequest));
	}
	...
	protected void handleMessage(Message message) {
		...
		case SUCCESS_MESSAGE:
			...
			if (response != null && response.length >= 3) {
				onSuccess((Integer) response[0], (Header[]) response[1],
						(byte[]) response[2], (HttpResponse)response[3]);
			} else {
		...
		case FAILURE_MESSAGE:
			...
			if (response != null && response.length >= 4) {
				if (null != mIErrorListener) {
					mIErrorListener.onFailure((Integer) response[0],
							(Header[]) response[1], (byte[]) response[2],
		...
		case START_MESSAGE: {
			...
			if (null != mIStateListener) {
				mIStateListener.onStart(asyncHttpRequest);
			} else {
				onStart(asyncHttpRequest);
			}
		}
			break;
		case FINISH_MESSAGE: {
			AsyncHttpRequest asyncHttpRequest = (AsyncHttpRequest) message.obj;
			if (null != mIStateListener) {
				mIStateListener.onFinish(asyncHttpRequest);
			} else {
				onFinish(asyncHttpRequest);
			}
		}
			break;
		case PROGRESS_MESSAGE:
			...
			if (response != null && response.length >= 2) {
				try {
					if (null != mIStateListener) {
						mIStateListener.onProgress((Integer) response[0],
								(Integer) response[1]);
					} else {
		...
		case RETRY_MESSAGE:
			...
			if (response != null && response.length == 1) {
				if (null != mIStateListener) {
					mIStateListener.onRetry((Integer) response[0]);
				} else {
			...
			if (!Thread.currentThread().isInterrupted()) {
				if (status.getStatusCode() >= 300) {
					...
					sendSuccessMessage(status.getStatusCode(),
							response.getAllHeaders(), responseBody,response);
				}
			}
		}
	}
